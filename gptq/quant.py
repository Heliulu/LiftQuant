    ...
    def find_params(self, x, weight=False):
        ...
        if self.maxq < 0:
          ...
          self.zero = xmin
            ...
            if self.scaleflag:
                print('Use trained scale')
                if self.sym :
                    self.zero = torch.full_like(self.scale, self.maxq / 2)  ### not integer! 0.5 1 1.5 for 1bit 1.58bit 2bit
                else:
                    self.zero = torch.round(-xmin / self.scale)
            else:
                self.scale = (xmax - xmin) / self.maxq
                
                #self.scale = (3*x.std(dim=1))/self.maxq
                #print(self.scale.shape)
                if self.sym :
                    #self.zero = torch.full_like(self.scale, (self.maxq + 1) / 2)
                    self.zero = torch.full_like(self.scale, self.maxq / 2)  ### not integer!
                else:
                    self.zero = torch.round(-xmin / self.scale)
